  ...
  const fetchPoolData = useCallback(async () => {
    ...
    try {
      const pool = await retryWithDelay(() => 
        stakeContract.read.pools([Pid]) as Promise<PoolData>
      );
  ...
  const fetchMetaNodeAddress = useCallback(async () => {
    ...
    try {
      const address = await retryWithDelay(() => 
        stakeContract.read.metaNodeToken() as Promise<string>
      );
  ...
  const fetchRewardsData = useCallback(async () => {
    ...
    try {
      ...
      const userData = await retryWithDelay(() =>
          stakeContract.read.users([Pid, address]) as Promise<UserData>
      );
